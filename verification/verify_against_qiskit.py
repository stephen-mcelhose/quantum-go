#!/usr/bin/env python3
"""
Verification script to compare Strange quantum operations against Qiskit.
This script runs each quantum operation from the learning transcript in both
Strange and Qiskit, then compares the results.
"""
...
class QuantumVerifier:
    """Compares quantum operations between Strange and Qiskit."""
    
    def __init__(self, strange_path: str = "./go/strange"):
        self.strange_path = strange_path
        self.simulator = AerSimulator(method='statevector')
        self.tolerance = 1e-4  # Tolerance for probability comparison
        
        # Check if Strange binary exists
        if not os.path.exists(self.strange_path):
            print("\n" + "="*70)
            print(" ERROR: Strange binary not found")
            print("="*70)
            print(f"\nCannot find Strange binary at: {self.strange_path}")
            print("\nTo fix this:")
            print("  1. Build Strange: cd go && go build -o strange ./cmd/strange")
            print("  2. Run from repo root: python3 go/verification/verify_against_qiskit.py")
            print("  3. Or specify path: python3 verify_against_qiskit.py /path/to/strange")
            print("="*70 + "\n")
            sys.exit(1)
        
    def run_strange(self, num_qubits: int, gates: str = None, 
                    circuit: str = None, params: Dict[str, str] = None) -> Dict[str, float]:
        """
        Run a quantum circuit in Strange and return probability distribution.
        ...
        try:
            ...
            return self._parse_strange_output(result.stdout)
        except FileNotFoundError:
            print(f"\nERROR: Strange binary not found at: {self.strange_path}")
            print("Build it with: cd go && go build -o strange ./cmd/strange")
            return {}
        except subprocess.CalledProcessError as e:
            print(f"Error running Strange: {e.stderr}")
            return {}
    
    def _parse_strange_output(self, output: str) -> Dict[str, float]:
        """Parse Strange CLI output to extract probability distribution."""
        probs = {}
    ...
    def compare_results(self, strange_probs: Dict[str, float], 
                       qiskit_probs: Dict[str, float], 
                       test_name: str) -> bool:
        """
        Compare probability distributions from Strange and Qiskit.
        
        This method performs a detailed comparison of quantum measurement probabilities
        from both simulators to verify that Strange produces correct results.
        ...
        2. For each state, compares the probability values:
           - Strange probability: P_s(|ψ⟩) = |⟨ψ|φ_s⟩|²
        ...
        Args:
            strange_probs: Dict mapping states like "|00⟩" to probabilities [0,1]
        ...
        Side effects:
            ...
            - Probability from Strange simulator
        ...
        #   Strange:  {|00⟩: 0.5, |11⟩: 0.5}        ← Correct
        #   Qiskit:   {|00⟩: 0.5, |01⟩: 0.3, |11⟩: 0.2}  ← Bug: extra state!
        #
        # If we only checked Strange's states, we'd miss that Qiskit produced |01⟩
        # If we only checked Qiskit's states, we'd miss if Strange had extra states
        #
        ...
        all_states = set(strange_probs.keys()) | set(qiskit_probs.keys())
        
        match = True  # Assume success until we find a mismatch
        print(f"\n{'State':<10} {'Strange':<12} {'Qiskit':<12} {'Diff':<12} {'Match'}")
        print("-" * 60)
        
        for state in sorted(all_states):
            # Get probability for this state (default to 0.0 if missing)
            strange_prob = strange_probs.get(state, 0.0)
            qiskit_prob = qiskit_probs.get(state, 0.0)
            
            # Calculate absolute difference
            diff = abs(strange_prob - qiskit_prob)
            ...
            print(f"{state:<10} {strange_prob:>10.4f}   {qiskit_prob:>10.4f}   "
                  f"{diff:>10.6f}   {status}")
    ...
    def verify_test(self, test_name: str, num_qubits: int, 
                    strange_gates: str = None, qiskit_gates: List[Tuple] = None,
                    circuit: str = None, params: Dict[str, str] = None) -> bool:
        """Run a single verification test."""
        strange_probs = self.run_strange(num_qubits, strange_gates, circuit, params)
        qiskit_probs = self.run_qiskit(num_qubits, qiskit_gates)
        return self.compare_results(strange_probs, qiskit_probs, test_name)


def main():
    """Run all verification tests from the learning transcript."""
    parser = argparse.ArgumentParser(
        description='Verify Strange quantum simulator against IBM Qiskit',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 verify_against_qiskit.py                    # Use default path ./go/strange
  python3 verify_against_qiskit.py ../strange         # Use relative path
  python3 verify_against_qiskit.py /usr/local/bin/strange  # Use absolute path
        """
    )
    parser.add_argument(
        'strange_path',
        nargs='?',
        default='./go/strange',
        help='Path to the Strange binary (default: ./go/strange)'
    )
    
    args = parser.parse_args()
    
    verifier = QuantumVerifier(args.strange_path)
    ...
    tests = [
        ...
        },
        {
            ...
            "num_qubits": 1,
            "strange_gates": "h q[0]; z q[0]; h q[0]",
        ...
        # Objective 6: Gate Compositions
        {
            "name": "X then Z",
            "num_qubits": 1,
            "strange_gates": "x q[0]; z q[0]",
            "qiskit_gates": [("x", [0]), ("z", [0])]
        },
        {
            "name": "Z then X",
            "num_qubits": 1,
            "strange_gates": "z q[0]; x q[0]",
            "qiskit_gates": [("z", [0]), ("x", [0])]
        },
        {
            "name": "H-sandwich with X (H·X·H)",
            "num_qubits": 1,
            "strange_gates": "h q[0]; x q[0]; h q[0]",
        ...
        },
        {
            ...
            "num_qubits": 1,
            "strange_gates": "x q[0]; y q[0]; z q[0]",
        ...
        # Examples from 'strange inspect' help
        {
            "name": "Inspect Example: 3-qubit GHZ state",
            "num_qubits": 3,
            "strange_gates": "h q[0]; cx q[0], q[1]; cx q[1], q[2]",
            "qiskit_gates": [("h", [0]), ("cx", [0, 1]), ("cx", [1, 2])]
        },
        
        # Examples from 'strange export' help
        {
        ...
        # Examples from 'strange verify' help
        {
            "name": "Verify Example: H + Phase rotation",
            "num_qubits": 2,
            "strange_gates": "h q[0]; u1(0.785398) q[0]",
            "qiskit_gates": [("h", [0]), ("u1", [0], 0.785398)]
        },
        
        # Examples from 'strange analyze' help
        {
    ...
    print("\n" + "="*70)
    print(" STRANGE vs QISKIT VERIFICATION SUITE")
